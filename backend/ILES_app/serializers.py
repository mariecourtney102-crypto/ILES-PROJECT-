...
class CustomUserSerializer(serializers.ModelSerializer):
    course_title = serializers.CharField(required=False, write_only=True)
    university_name = serializers.CharField(required=False, write_only=True)
    year_of_study = serializers.IntegerField(required=False, write_only=True)
    place_of_work = serializers.CharField(required=False, write_only=True)
    department = serializers.CharField(required=False, write_only=True)
    staff_ID = serializers.CharField(required=False, write_only=True)
    ...
    def create(self, validated_data):
        ...
        if role == "student":
            Student.objects.create(users=user, **student_data)
        elif role == "supervisor":
            Supervisor.objects.create(users=user, **supervisor_data)
        elif role == "admin":
            Admin.objects.create(users=user, **admin_data)
